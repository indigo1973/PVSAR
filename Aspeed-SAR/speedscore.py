...
def error_translation_tensor(t_pr, t_gt, applyThresh,  posThresh=0.002173):
    # import pdb;pdb.set_trace()
    Et_norm = torch.sqrt(torch.sum((t_pr - t_gt) ** 2, dim=1))/torch.sqrt(torch.sum((t_gt**2), dim=1))
    Et = torch.sqrt(torch.sum((t_pr - t_gt) ** 2, dim=1))
    Et_mean = Et.mean()

    if applyThresh:
        Et_norm = Et_norm.masked_fill(Et_norm < posThresh, 0.0)

    Et_norm_mean = Et_norm.mean()
    Et_norm_max = Et_norm.max()
    return Et_norm_mean, Et_norm_max, Et_mean

def error_orientation_tensor(q_pr, q_gt, applyThresh, rotThresh=0.00295):    #[RAD]
    cos = torch.sum(torch.abs(q_pr * q_gt), dim = 1)
    Eq = 2 * torch.acos(cos.clamp(-1+1e-6, 1-1e-6))
    # import pdb;pdb.set_trace()
    # Eq = 2 * torch.acos(min(cos,1.0))

    if applyThresh:
        Eq = Eq.masked_fill(Eq < rotThresh, 0.0)

    Eq_mean = Eq.mean()
    Eq_max = Eq.max()
    return Eq_mean, Eq_max

def compute_ESA_score(t_pr, q_pr, t_gt, q_gt, applyThresh=True):
    # import pdb; pdb.set_trace()

    speed_tr, speed_tr_max, Etr = error_translation_tensor(t_pr, t_gt, applyThresh)   #[m]

    speed_q, speed_q_max = error_orientation_tensor(q_pr, q_gt, applyThresh)   #[RAD]
    speed_score = speed_tr + speed_q

    speed = speed_score.cpu().numpy()
    speed_t = speed_tr.cpu().numpy()
    speed_r = speed_q.cpu().numpy()
